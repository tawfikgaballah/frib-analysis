...
// Create an RDataFrame to read from the tree
    ...
    // auto df_filtered = df.Filter({"corr.isimplant==1 || corr.isdecay==1"});
	    data->GetBranch("clover.ecal[52]")->SetNameTitle("clover_ecal", "clover_ecal");
...
// df_mod = df_mod.Define("clover_timecal_arr", [](const std::array<double, 52>& timecal) {
//     std::array<double, 52> timecal_arr;  // Define a std::array of 52 doubles
//     for (int i = 0; i < 52; ++i) {
//         timecal_arr[i] = timecal[i];  // Copy values into the array
//     }
//     return timecal_arr;  // Return the array
// }, {"clover_timecal"});
    ...
    df.Snapshot("trimmed_data", Form("/mnt/analysis/e21062/sortedData2024/betasorted/localAnalyzed/TG/filtered_files/%s", filename.c_str()), 
	      {"implantlist", "corr.dxpos", "corr.dypos", "corr.dtime", 
	       ...
	       "pspmtlow1.dycfdfailbit", "pspmthigh1.dycfdfailbit","clover_cfdfailbit", "clover_cfdfailbit", "clover_timecal"});
