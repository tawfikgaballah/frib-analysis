...
void Reader_Mine(){
  ...
  // Open the directory
    ...
    while ((file = gSystem->GetDirEntry(dir))) {
        ...
        if (fileName.BeginsWith("run-") && fileName.EndsWith("-xscintT-sipm-sorted-TG-48-10mm.root")) {
    ...
    for (int runNumber : runNumbers) {
        TString filePath = TString::Format("/mnt/analysis/e21062/sortedData2024/betasorted/localAnalyzed/TG/run-%d-xscintT-sipm-sorted-TG-48-10mm.root", runNumber);
...
for (const auto& entry : nuclei) {
   ...
   TFile *output =new TFile("PID.root","recreate");
