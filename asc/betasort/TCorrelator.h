...
class TCorrelator
{
 public:
  TCorrelator();
  void Reset();
  int Correlate(TBetaDecayParams &bdecay);
  void clearImplants() { fCorrelatedImplants.clear(); }
  std::vector<TPID> getImplants() { return fCorrelatedImplants; }
  
 private:
  TImplant fImplant[npspmt_CeBr][npspmt_CeBr];
  std::vector<TImplant> fImplantVector[npspmt_CeBr][npspmt_CeBr];
  const int fCorrWin = 6; // Correlation window size in pixels
