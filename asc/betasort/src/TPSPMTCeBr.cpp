...
// 
void TPSPMTCeBrCalibrator::Initialize()
{
  // dynode
  dyoffset = 0.;
  dyslope = 1.;
...
//   - none
//
void TPSPMTCeBrCalibrator::ReadCalFile(std::string name)
{
  ...
  if(!calfile) {
	...
	if(linenum < 4*nanodes_CeBr+1) {
	  std::stringstream linestream(line);
	  linestream >> location >> value;
  	  if(linenum < nanodes_CeBr+1) aslope[location-1] = value;
  	  else if(linenum < 2*nanodes_CeBr+1) aoffset[location-1] = value;
  	  else if(linenum < 3*nanodes_CeBr+1) athresh[location-1] = value;
  	  else if(linenum < 4*nanodes_CeBr+1) auld[location-1] = value;
  	} else {
	  std::stringstream linestream(line);
	  linestream >> value;
	  if(linenum == 4*nanodes_CeBr+1) dyslope = value;
  	  if(linenum == 4*nanodes_CeBr+2) dyoffset = value;
	  if(linenum == 4*nanodes_CeBr+3) dyfitslope = value;
  	  if(linenum == 4*nanodes_CeBr+4) dyfitoffset = value;
	  if(linenum == 4*nanodes_CeBr+5) dyclockcal = value;
  	  if(linenum == 4*nanodes_CeBr+6) dythresh = value;
  	  if(linenum == 4*nanodes_CeBr+7) dyuld = value;
	  if(linenum == 4*nanodes_CeBr+8) dycoincwindow = value;
	}
      }
    }
  }
...
//   - none
//
void TPSPMTCeBrCalibrator::ReadTimeCalFile(std::string name)
{
  ...
  if(!calfile) {
	...
	if(linenum < nanodes_CeBr+1) tdiffdynode[location-1] = value;
      } else {
	//ignore line
	calfile.ignore(1000,'\n');
      }
...
//   - none
//
void TPSPMTCeBr::UnpackDynode(DDASRootFitHit* ddashit, TRandom3 &rng, TPSPMTCeBrCalibrator &cal)
{
  ...
  dycfdfailbit = ddashit->getTimeCFD(); // no get();
...
//   - none
//
void TPSPMTCeBr::UnpackDynodeFit(DDASRootFitHit* ddashit, TPSPMTCeBrCalibrator &cal, int i)
{
...
const int rows = 16; // Number of rows
const int cols = 16; // Number of columns
const double grid[rows][cols] = {

				 {51, 58, 69, 76, 74, 78, 79, 69, 57, 87, 88, 92, 80, 69, 87, 50},
				 {69, 72, 70, 69, 68, 70, 71, 76, 75, 80, 81, 83, 81, 83, 87, 61},
				 {73, 68, 62, 63, 61, 61, 63, 67, 74, 78, 78, 78, 77, 82, 85, 68},
				 {73, 65, 61, 61, 60, 60, 61, 63, 73, 78, 78, 77, 77, 80, 85, 66},
				 {72, 66, 60, 61, 60, 58, 59, 60, 70, 75, 77, 77, 77, 79, 83, 68},
				 {75, 67, 61, 59, 58, 60, 57, 55, 67, 72, 76, 75, 76, 78, 83, 67},	
				 {77, 69, 64, 60, 56, 61, 55, 52, 64, 70, 71, 73, 76, 78, 84, 70},
				 {80, 71, 65, 58, 56, 58, 54, 51, 62, 67, 69, 72, 75, 77, 85, 68},
				 {81, 73, 64, 57, 55, 58, 54, 52, 60, 64, 69, 72, 73, 78, 84, 69},
				 {82, 73, 64, 58, 58, 60, 57, 52, 62, 65, 70, 72, 73, 76, 84, 68},
				 {83, 75, 66, 62, 63, 62, 59, 57, 65, 68, 73, 72, 71, 75, 82, 71},	
				 {84, 75, 68, 64, 65, 63, 62, 59, 68, 69, 72, 72, 72, 74, 81, 68},
				 {84, 76, 69, 68, 69, 67, 67, 68, 75, 73, 74, 75, 73, 74, 81, 70},
				 {88, 79, 72, 72, 74, 72, 72, 72, 77, 79, 78, 78, 76, 77, 83, 69},
				 {92, 74, 67, 77, 79, 78, 77, 78, 78, 81, 81, 84, 85, 88, 93, 79},
				 {78, 78, 79, 87, 87, 88, 88, 91, 90, 88, 82, 90, 89, 89, 100, 72}
};
void TPSPMTCeBr::UnpackAnode(DDASRootFitHit* ddashit, TRandom3 &rng, TPSPMTCeBrCalibrator &cal, int pix)
{
  // extract hit information
  ...
  yPos =(int)(pix)/16;
  int xPos1=xPos;
  int yPos1=abs(yPos-15);
  double energy = (ddashit->getEnergy() + rng.Rndm())/(grid[yPos1][xPos1]/100);
  aenergy[pix] = energy;
   if(pix==212){
    aenergy[pix]=0;
  }
  ...
  if(!dytime.empty() && std::abs(timecal - dytime.front()) < cal.dycoincwindow) {
...
//   - none
//
void TPSPMTCeBr::FitAnodePosition()
{
  TLinearSolver solver;
  solver.Reset();
  double anodexproj[npspmt_CeBr] = {0.}; // x projection of 2D energy plot
  double anodeyproj[npspmt_CeBr] = {0.}; // y projection of 2D energy plot

  // fill the projection histograms
  for(int i=0; i<nanodes_CeBr; i++) {
    if(aenergy[i] > 0) {
      int xpix = i % npspmt_CeBr;
      int ypix = i / npspmt_CeBr;
      xpix = i % npspmt_CeBr;
      ypix = abs((i / npspmt_CeBr)-15);
      if(ypix > 0 && ypix < npspmt_CeBr) {
	anodexproj[xpix] += aenergy[i];
	anodeyproj[ypix] += aenergy[i];
      }
  ...
  for(int i=0; i<npspmt_CeBr; i++) { // data bins only
    if(anodexproj[i] > 0) {
      double x = i + 0.5;
