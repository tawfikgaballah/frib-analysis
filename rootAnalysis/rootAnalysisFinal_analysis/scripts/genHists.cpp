/* Script to make some general histograms, not gated on a specific decay*/
...
void genHists(){
	...
	string gateName = "ungated";

	// Get gates
	
	//cout << "Instantiating " << gateName << " decay gate.." << endl;
   
	/*
    TCutG *pidGate = new TCutG("pidGate",7);
    pidGate->SetVarX("(implantlist[0].tof)");
    pidGate->SetVarY("(implantlist[0].de2)");
    pidGate->SetTitle("Graph");
    pidGate->SetFillStyle(1000);
    pidGate->SetPoint(0,-277.718,152.645);
    pidGate->SetPoint(1,-254.474,5444.9);
    pidGate->SetPoint(2,-244.11,5763.61);
    pidGate->SetPoint(3,-234.203,4404.9);
    pidGate->SetPoint(4,-203.339,77.1612);
    pidGate->SetPoint(5,-277.032,77.1612);
    pidGate->SetPoint(6,-277.718,152.645);
	*/
	...
	//TH2D *ecal_vs_time = new TH2D("clover_ecal_vs_time", Form("Clover energy versus time of #gamma since time of #beta, gated on %s decays", gateName.c_str()), 1000, -10, 10, 4000, 1, 4001);  // 1 keV, 20 ns binning
			...
			//if(pidGate->IsInside((implantlist->at(0)).tof, (implantlist->at(0)).de2)){
				for(int j = 0; j < 52; j++){
					if(clover_ecal[j] > thresh){	
						...
						//ecal_vs_time->Fill((clover_time[j]/1000 - *corr_dtime*1000), clover_ecal[j]);
	...
	fileOutPath << "/projects/e21062/aad/e21062B/rootAnalysisFinal/histograms/bDecay/";
	fileOutPath << gateName;
	fileOutPath << ".root";

	TFile *fileOut = new TFile(fileOutPath.str().c_str(), "RECREATE");

	cout << endl << "Writing output to " << fileOutPath.str() << endl;

	// Draw histograms
	TCanvas *c0 = new TCanvas("c0", "c0", 800, 600);
	c0->cd();
    double ecalBinWidth = (ecal->GetXaxis()->GetXmax()-ecal->GetXaxis()->GetXmin())/ecal->GetXaxis()->GetNbins();
	ecal->GetXaxis()->SetTitle("keV");
	ecal->GetYaxis()->SetTitle(Form("counts / %4.2F keV", ecalBinWidth));
	ecal->Draw("hist");

	/*
	TCanvas *c1 = new TCanvas("c1", "c1", 800, 600);
	c1->cd();
    double timeBinWidth = (ecal_vs_time->GetXaxis()->GetXmax()-ecal_vs_time->GetXaxis()->GetXmin())/ecal_vs_time->GetXaxis()->GetNbins();
    ecalBinWidth = (ecal_vs_time->GetYaxis()->GetXmax()-ecal_vs_time->GetYaxis()->GetXmin())/ecal_vs_time->GetYaxis()->GetNbins();
	ecal_vs_time->GetXaxis()->SetTitle(Form("counts / %5.2f #mus", timeBinWidth));
	ecal_vs_time->GetYaxis()->SetTitle(Form("counts / %4.2f keV", ecalBinWidth));
	ecal_vs_time->Draw("colz");
	*/

	TCanvas *c4 = new TCanvas("c4", "c4", 800, 600);
	c4->cd();
    ecalBinWidth = (ecal_vs_detector->GetXaxis()->GetXmax()-ecal_vs_detector->GetXaxis()->GetXmin())/ecal_vs_detector->GetXaxis()->GetNbins();
	ecal_vs_detector->GetYaxis()->SetTitle("Detector number");
	ecal_vs_detector->GetXaxis()->SetTitle(Form("Clover energy (counts / %4.2f keV)", ecalBinWidth));
