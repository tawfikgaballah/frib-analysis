...
void drawPID(){
	...
	if(pidcorr.is_open()){
		...
		if(flag == false){
			//std::cout << "Error: No PID Correction found for run " << runNum << ". Exiting." << std::endl;
			//exit(1);
			std::cout << "Warning: No PID Correction found for run " << runNum << ". Setting TOF offset = 0 and deScale = 1." << std::endl;
		}
