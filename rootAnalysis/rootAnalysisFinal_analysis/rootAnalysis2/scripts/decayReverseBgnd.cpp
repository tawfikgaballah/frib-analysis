double expDecay(double *x, double *p){
	return p[0]*pow(0.5, x[0]/p[1])+p[2]*pow(0.5, x[0]/p[3]);
}

void decayReverseBgnd(){

	string parentName = "30Ne";

	TFile *fRvrs = new TFile(Form("/projects/e21062/aad/e21062B/rootAnalysisFinal/myHistos/reverse/d%s-37ms-2mm-reverse.root", parentName.c_str()));
	...
	for(int i = 1; i <= nBinsBg; i++){
		bgndShift->SetBinContent(i, bgnd->GetBinContent(i));
	}

	TF1 *decayFunc = new TF1("decayFunc", expDecay, 0, 1750, 4);
	decayFunc->SetParameters(300, 150, 1850, 100000);
	...
	c2->cd();
	bgndShift->GetXaxis()->SetTitle("abs(#beta time - implant time), ms");
	bgndShift->GetYaxis()->SetTitle(Form("counts / %3.2f ms", bgndShift->GetBinWidth(3)));
	bgndShift->Draw();
	decayFunc->Draw("same");
