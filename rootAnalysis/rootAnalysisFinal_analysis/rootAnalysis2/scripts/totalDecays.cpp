void totalDecays(){
	...
	TH1D *allDecays = new TH1D("allDecays", Form("%s total #beta-decay curve", gateName.c_str()), 350, -1750, 0);  // 5 ms binning	
	...
	string output_file_path = "/projects/e21062/aad/rootAnalysisFinal/histograms/bDelayed/";
	TFile *fout = new TFile(Form("%sd%s-totalDecays-reverse.root",output_file_path.c_str(), gateName.c_str()), "RECREATE");

	cout << "Writing output to " << output_file_path << "d" << gateName << ".root" << endl;

	// Draw histograms
	TCanvas *c0 = new TCanvas("c0", "c0", 800, 600);
	c0->cd();
	allDecays->GetYaxis()->SetTitle("counts / 5 ms");
