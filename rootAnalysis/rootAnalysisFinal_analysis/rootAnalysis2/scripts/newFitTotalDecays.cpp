...
//	5 Beta-n HL; 6 Beta-2n HL; 7 Pbn; 8 Pb2n; 9 bg intercept; 10 bg slope
...
Double_t bgnd(Double_t *x, Double_t *p){return p[0]*pow(0.5, x[0]/p[1])+p[9]*pow(0.5, x[0]/p[10]);}
...
Double_t reverseBG(Double_t *x, Double_t *p){return p[0]*pow(0.5, x[0]/p[1])+p[2]*pow(0.5, x[0]/p[3]);}
...
DecayData getDecayData(string parentName){
	...
	fittingDataFile.open("/projects/e21062/aad/e21062B/rootAnalysisFinal/scripts/fittingDataFile.txt");	
...
void residuals(TF1 *fitFunc, TH1D *htime, int maxBin){
...
vector<double> getBgHL(string parentName){
	TFile *fBG = new TFile(Form("/projects/e21062/aad/e21062B/rootAnalysisFinal/myHistos/reverse/d%s-37ms-2mm-reverse.root", parentName.c_str()));
	...
	for(int i = 1; i <= nBinsBg; i++){
		...
		bgShift->SetBinError(i, sqrt(histBG->GetBinContent(i)));
	}

	TF1 *bgFunc = new TF1("bgFunc", reverseBG, 0, 1750, 4);
	bgFunc->SetParameters(300, 150, 1850, 100000);
	


	

	bgShift->Fit(bgFunc, "QNWL");


	double bgA1 = bgFunc->GetParameter(0);
	double bgHL1 = bgFunc->GetParameter(1);
	double bgA2 = bgFunc->GetParameter(2);
	double bgHL2 = bgFunc->GetParameter(3);

	vector<double> result = {bgA1, bgHL1, bgA2, bgHL2};
	...
	bgShift->SetTitle(Form("Background HL = %4.1f ms, %4.1f ms", bgHL1, bgHL2));
	bgShift->Draw("hist");
	bgFunc->Draw("same");
	...
	cout << "Background half-life = " << bgHL1 << " ms, " << bgHL2 << " ms" << endl;
...
void newFitTotalDecays(){
	...
	vector<double> bg = getBgHL(parentName);
	double bgA1 = bg.at(0);
	double bgHL1 = bg.at(1);
	double bgA2 = bg.at(2);
	double bgHL2 = bg.at(3);

	//TFile *f = new TFile("/projects/e21062/aad/e21062B/rootAnalysisFinal/myHistos/bDecay/d32Ne-18ms-2mm.root");
	TFile *f = new TFile(Form("/projects/e21062/aad/e21062B/rootAnalysisFinal/myHistos/bDecay/d%s-37ms-2mm.root", parentName.c_str()));
	TH1D *htime = (TH1D*)f->Get("allDecays");

	int max = 37;
	double binWidth = htime->GetXaxis()->GetBinWidth(2);
	int npts = max/binWidth;
	double newBinWidth;
	...
	htime->GetXaxis()->SetRangeUser(0, max);
	htime->Draw("hist");
	...
	fitFunc->SetParameter(0, bgA1);	
	fitFunc->FixParameter(1, bgHL1);
	fitFunc->SetParameter(9, bgA2);
	fitFunc->FixParameter(10, bgHL2);
	fitFunc->SetParameter(2, 5);
	fitFunc->SetParameter(3, 400);
	...
	htime->GetYaxis()->SetTitle(Form("counts / %4.2f ms", newBinWidth));
	htime->GetXaxis()->SetRangeUser(0, max);
	fitFunc->Draw("same");
...
//	5 Beta-n HL; 6 Beta-2n HL; 7 Pbn; 8 Pb2n
	...
	cout << "Bgnd constant = " << fitFunc->GetParameter(9) << " +/- " << fitFunc->GetParError(9) << " counts / " << newBinWidth << " ms" << endl;
