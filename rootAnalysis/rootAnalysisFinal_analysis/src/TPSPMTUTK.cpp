...
#include "/projects/e21062/tg/rootAnalysis/rootAnalysisFinal/ddastoys/DDASToys/functions_analytic.h"   // need to add explicitly because of
				  // DDAS AnalyticFit namespace
#include "/projects/e21062/tg/rootAnalysis/rootAnalysisFinal/ddastoys/DDASToys/lmfit_analytic.h"	 // need to add explicitly because of
				  // DDAS AnalyticFit namespace
...
//	std::cout << "aenergy_qdc["<<pix << "]" << aenergy_qdc[pix] << std::endl;
	}
	aecal[pix] = 99999.;
	asum += 99999.;
	atime[pix] = time;
...
//   - none
//
void TPSPMTUTK::FitAnodePosition()
{
  ...
  if(check == true && dytrace.size()>0) {
    double xa_high = aenergy_qdc[0];
    double xb_high = aenergy_qdc[1];
     double ya_high = aenergy_qdc[2];
    double yb_high = aenergy_qdc[3];
double kLOW_GAIN_THRESHOLD=-900; 
double kLOW_GAIN_OVERFLOW=6553500;

   if(xa_high>kLOW_GAIN_THRESHOLD && // kLOW_GAIN_THRESHOLD = -900
            xb_high>kLOW_GAIN_THRESHOLD && 
            ya_high>kLOW_GAIN_THRESHOLD &&
            yb_high>kLOW_GAIN_THRESHOLD &&
            xa_high<kLOW_GAIN_OVERFLOW &&  // kLOW_GAIN_OVERFLOW = 6553500
            xb_high<kLOW_GAIN_OVERFLOW &&
            ya_high<kLOW_GAIN_OVERFLOW &&
            yb_high<kLOW_GAIN_OVERFLOW){


//     xpos_highgain =npspmt_utk*(ya_high + xb_high)/asum_qdc;
//     ypos_highgain = npspmt_utk*(xa_high + xb_high)/asum_qdc;
//     double x_min = 9;// Minimum observed or assumed value
// double x_max = 39;// Maximum observed or assumed value

// double a = npspmt_utk / (x_max - x_min);
// double b = -a * x_min;


//     xpos_highgain = xpos_highgain * a +b;
// 		ypos_highgain = ypos_highgain * a +b;
...
xpos_highgain = xpos_highgain * 1.6 - 7.8;
ypos_highgain = ypos_highgain * 1.5 - 6.0;
  
/* double step=.01;
for(double i=0; i<1; i+=step){
if(xpos_highgain>=i && xpos_highgain<i+step){
...
// Now apply this transformation to each data point
 xpos_highgain= a * (xpos_highgain) + b;
 break;
}
}
for(double i=0; i<1; i+=step){
if(ypos_highgain>=i && ypos_highgain<i+step){
...
// Now apply this transformation to each data point
 ...
 break;
    ...
    if(xpos_highgain < 0 || xpos_highgain > npspmt_utk)
      xpos_highgain = ERR_BADPOS;
    if(ypos_highgain < 0 || ypos_highgain > npspmt_utk)
      ypos_highgain = ERR_BADPOS;
  } else {
    xpos = ERR_QDCMULT;
    ypos = ERR_QDCMULT;
    xpos_highgain = ERR_QDCMULT;
    ypos_highgain = ERR_QDCMULT;
  }
