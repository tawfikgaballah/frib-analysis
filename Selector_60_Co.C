...
#define Selector_60_Co_cxx
// The class definition in Selector_60_Co.h has been generated automatically
...
//
// root> T->Process("Selector_60_Co.C")
// root> T->Process("Selector_60_Co.C","some options")
// root> T->Process("Selector_60_Co.C+")
//

#include "Selector_60_Co.h"
#include <TH2.h>
#include <TStyle.h>
...
void Selector_60_Co::Begin(TTree * /*tree*/)
{
...
void Selector_60_Co::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t Selector_60_Co::Process(Long64_t entry)
{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either Selector_60_Co::GetEntry() or TBranch::GetEntry()
  ...
  double time=pspmthigh2_dytime[0];
  double energy=pspmthigh2_dyenergy[0];
  double cfdfailbit=*pspmthigh2_dycfdfailbit;
  for(int i=0; i<16; i++){
    if(labr_ecal[i]>0 && labr_timecal[i]>10 && labr_cfdfailbit[i]!=1 &&cfdfailbit!=1 && (energy)>10 && (time)>10){
      Double_t TDynode=labr_timecal[i]-(time);
...
void Selector_60_Co::SlaveTerminate()
{
