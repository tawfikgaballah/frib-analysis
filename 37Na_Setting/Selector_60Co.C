...
const int t_min=-1000000;
const int t_max=10000000;
...
Bool_t Selector_60Co::Process(Long64_t entry)
{
  ...
  double dytime;
  double dyenergy;
  if( pspmthigh2_dyenergy.size()>0 && pspmthigh2_dytime.size()>0){
    // for(int i=0; i<3; i++){
    // if(pspmthigh2_dytime[]>dytime){
    dytime=pspmthigh2_dytime[0];
    
    // // }

    // if(pspmthigh2_dyenergy[i]>dyenergy){
    dyenergy=pspmthigh2_dyenergy[0];
    std::cout<<dytime<<"   "<<dyenergy<<'\n';
    // }
    // }
    ...
    for(int i=0; i<16; i++){
      if(labr_ecal[i]>0 && labr_timecal[i]>10 && labr_cfdfailbit[i]!=1 &&pspmthigh2_dycfdfailbit!=1 && dyenergy>10 && dytime>10){
	Double_t TDynode=labr_timecal[i]-dytime;
