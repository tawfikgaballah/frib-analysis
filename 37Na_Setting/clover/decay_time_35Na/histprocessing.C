...
void histprocessing(){
  ...
  canvas->Divide(3);
  canvas->cd(1);
TFile* file=new TFile("./../position_Vs_decaytime_37Na_33Na_Decay_3mmReverse.root","READ");
  TH2D* hist2d=(TH2D*)file->Get("Position_Vs_hIonDecayTime_33Na");
TH1D* myHist=(TH1D*)hist2d->ProjectionX();
// Now flip the histogram
 TH1D* myHist_37=(TH1D*) FlipHistogramSign(myHist);

 TFile* file1=new TFile("./../position_Vs_decaytime_37Na_33Na_Decay_3mm.root","READ");
 TH2D* hist2d1=(TH2D*)file1->Get("Position_Vs_hIonDecayTime_33Na");
TH1D* myHist1_37=(TH1D*)hist2d1->ProjectionX();
 ...
 myHist1_37->SetName("Position_Vs_hIonDecayTime_33Na_Reverse");
 myHist_37->SetName("Position_Vs_hIonDecayTime_33Na_Reverse");

 TFile* output=new TFile("./../position_Vs_decaytime_37Na_33Na_Decay_Forward-Reverse-3mm.root","UPDATE");
 myHist_37->Write();
