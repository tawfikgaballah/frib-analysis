...
void FitSingle4Efficiency(const string motherNucl = "33Na", 
  ...
  DecayCascade *dc = new DecayCascade(1500, motherNucl);
  ...
  printf("--- Total number of mother nucleus %s: \n\t%.1lf (<10.0x halflife (Total counts in the decay curve 0->80 ms)); \n\t%.1lf (<%.1lfx halflife (Integral of the parent component 0->40 ms))\n",
	 ...
	 (fMother->Integral(0, 40))//-hDecayTime->GetEntries() )/hDecayTime->GetEntries()
	 ,
	 NHALFLIFE 
	 ); 
...
pt->AddText(Form("^{33}Na: T_{1/2} = %.2lf #pm %.2lf ms", f->GetHalflife(), f->GetHalflifeErr()));
