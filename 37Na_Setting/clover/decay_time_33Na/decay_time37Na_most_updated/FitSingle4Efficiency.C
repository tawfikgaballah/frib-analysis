...
const string dir = "/projects/e21062/tg/37Na_Setting/clover/decay_time_33Na/decay_time33Na_most_updated/"; ///home/yongchi/data/research/nscl2018/e16032/process_root_files/Correlator2/";

// @UPDATE 04/2020
//const string rootName = "decaytime_35Na_34Na_Decay_Forward-Reverse-3mm.root"; 
//const string rootName ="33Na_Decay_Curve_nocorrection.root"; 
//const string rootName ="Amelia_Files_33_37_Na_PID_6mm.root";
const string rootName ="33_37_Na_Decay_Time_PID_descale_6mm_corr_radius.root";
// const string rootName = "position_Vs_decaytime_37Na_33Na_Decay_Forward-Reverse-3mm.root"; 
void FitSingle4Efficiency(const string motherNucl = "37Na", 
			  double NHALFLIFE = 4.0) {
  gStyle->SetOptStat(0000); 

  //_____Prepare decay cascade_____
  DecayCascade *dc = new DecayCascade(fitRange, motherNucl);
  ...
  f->Fit();
  std::cout<<"Bin Width Right After Drawing"<< hDecayTime->GetBinWidth(0)<<'\n';
  ...
  printf("--- Total number of mother nucleus %s: \n\t%.1lf (<10.0x halflife (Total counts in the decay curve 0->80 ms)); \n\t%.1lf (<%.1lfx halflife (Integral of the parent component 0->40 ms))\n",
	 motherNucl.c_str(),
	 // fTotal->GetParameter(1)*f->GetHalflife()/0.693,
	 // sqrt(fTotal->GetParError(1)*f->GetHalflife()/0.693)
	 // ig.Integral(0, f->GetHalflife()*50),
	 (fMother->Integral(0, 10)/hDecayTime->GetBinWidth(1)),
	 /* hDecayTime->Integral(hDecayTime->FindBin(0), hDecayTime->FindBin(80)), */
	 (fMother->Integral(0, 5)/hDecayTime->GetBinWidth(1))//-hDecayTime->GetEntries() )/hDecayTime->GetEntries()
	 ,
	 NHALFLIFE 
	 ); 
