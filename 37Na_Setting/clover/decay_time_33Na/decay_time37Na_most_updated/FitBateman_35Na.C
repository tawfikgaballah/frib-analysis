...
void FitBateman_35Na(const string nucleus_ = "35Na") {
  ...
  fExpBackground = new TF1("fExpBackground",
	...
	fExpBackgroundreverse = new TF1("fExpBackground",
							 batemanrate::ExpBackground,
							 -5,0, 12);
	fExpBackgroundreverse->SetParameters(par);
	fExpBackgroundreverse->SetLineColor(2);
	
		fTotal->FixParameter(0,par[0]);
		//	fTotal->SetParLimits(0, 0,1000);

			fTotal->SetParLimits(1, 1e0, 100000);
			//		fTotal->FixParameter(1,par[1]);
 
    fTotal->SetParLimits(2,par[2]/100,par[2]*100);// halflife

    fTotal->FixParameter(3,par[3]);
    //  fTotal->SetParLimits(3,0,nucl[0]->B1*2);
      fTotal->FixParameter(4,par[4]);
    // fTotal->SetParLimits(4,0,nucl[0]->B2*4);
          fTotal->FixParameter(5,par[5]);
	  // fTotal->SetParLimits(5,0,.25*4);
  ...
  // fTotal->SetParLimits(10,-1, par[10]*10);
  ...
  TH1D *hDecayTime = (TH1D*)roottools::GetDecayTimeDistribution("/projects/e21062/tg/37Na/DecayTime_Vs_Radius.root", "35Na");
  //hDecayTime->RebinX(5);
  ...
  p1->cd();
  p1->SetLogy();
  hDecayTime->SetTitle(";Time (ms);Activity / 0.5 ms"); 
  hDecayTime->Draw("hist PE");
   hDecayTime->SetMinimum(5e-1);

  auto fitresult1= hDecayTime->Fit(fTotal, "ROS");
  
  fTotal->Draw("same"); // this is where to the the parameters and errors
  TF1ToText(fTotal,"fTotal.dat", -10, 60, 300);
  for(int i = 0; i < 12; i++) {
    ...
    fExpBackgroundreverse->FixParameter(i,fTotal->GetParameter(i));
  }
  fTotal->Print("all");
 hDecayTime->Fit(fMother, "QROWL");
 TF1ToText(fMother,"fMother.dat", -10, 60, 300);
  fMother->Draw("same");
  hDecayTime->Fit(fDaughter1, "QROWL");
  fDaughter1->SaveAs("fDaughter1.txt");
   TF1ToText(fDaughter1,"fDaughter1.dat", -10, 60, 300);
  fDaughter1->Draw("same"); 
 hDecayTime->Fit(fDaughter2, "QROWL");
 fDaughter2->SaveAs("fDaughter2.txt");
  TF1ToText(fDaughter2,"fDaughter2.dat", -10, 60, 300);
  fDaughter2->Draw("same");
  hDecayTime->Fit(fDaughter3, "QROWL");
  fDaughter3->SaveAs("fDaughter3.txt");
   TF1ToText(fDaughter3,"fDaughter3.dat", -10, 60, 300);
  fDaughter3->Draw("same");

   hDecayTime->Fit(fDaughter4, "QROWL");
   fDaughter4->SaveAs("fDaughter4.txt");
    TF1ToText(fDaughter4,"fDaughter4.dat", -10, 60, 300);
  fDaughter4->Draw("same");
  hDecayTime->Fit(fExpBackground, "QROWL");
  fExpBackground->SaveAs("fExpBackground.txt");
   TF1ToText(fExpBackground,"fExpBackground.dat", 0, 60, 300);
  fExpBackground->Draw("same");
  TH1DToText(hDecayTime, "hist.dat");

  //   hDecayTime->Fit(fExpBackgroundreverse, "QROWL");
  // fExpBackgroundreverse->Draw("same");
