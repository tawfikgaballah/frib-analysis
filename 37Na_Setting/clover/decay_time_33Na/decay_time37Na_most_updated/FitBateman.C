...
void FitBateman(const string nucleus_ = "37Na") {
  ...
  fTotal->FixParameter(9,par[9]);
    fTotal->FixParameter(10,par[10]);
      ...
      fTotal->FixParameter(11,par[11]);
       // fTotal->SetParLimits(10,0, par[10]*10);
  ...
  //TH1D *hDecayTime = (TH1D*)roottools::GetDecayTimeDistribution("/projects/e21062/tg/37Na/DecayTime_Vs_Radius.root", "37Na");
  TH1D *hDecayTime = (TH1D*)roottools::GetDecayTimeDistribution("/mnt/analysis/e21062/tg/batch_analysis/analyzed/merged_decay_analysis.root", "37Na");
  //hDecayTime->RebinX(5);
  ...
  auto fitresult1= hDecayTime->Fit(fTotal, "ROLLS");
  
  fTotal->Draw("same"); // this is where to the the parameters and errors
    TF1ToText(fTotal,"fTotal_37Na.txt", -10, 60, 300);
  for(int i = 0; i < 12; i++) {
 ...
 hDecayTime->Fit(fMother, "QROWL");
  fMother->Draw("same");
   // TF1ToText(fMother,"fMother_37Na.txt", -10, 60, 300);
   hDecayTime->Fit(fDaughter1, "QROWL");
   fDaughter1->Draw("same");
 //    TF1ToText(fDaughter1,"fDaughter1_37Na.txt", 0, 60, 300);
  hDecayTime->Fit(fDaughter2, "QROWL");
   fDaughter2->Draw("same");
 //   TF1ToText(fDaughter2,"fDaughter2_37Na.txt", 0, 60, 300);
   hDecayTime->Fit(fDaughter3, "QROWL");
   fDaughter3->Draw("same");
 //  TF1ToText(fDaughter3,"fDaughter3_37Na.txt", 0, 60, 300);
    hDecayTime->Fit(fDaughter4, "QROWL");
   fDaughter4->Draw("same");
    TF1ToText(fDaughter4,"fDaughter4_37Na.txt", 0, 60, 300);
   hDecayTime->Fit(fExpBackground, "QROWL");
  fExpBackground->Draw("same");
 //   TF1ToText(fExpBackground,"fExpBackground_37Na.txt", 0, 60, 300);
  ...
  tl->AddEntry(fDaughter3->GetName(),
   ...
   tl->Draw("same");
  TH1DToText(hDecayTime, "hist_37Na.csv");
