...
const string rootName = "decaytime_CorrSorted37Na-Win3.root"; 
...
void FitSingle4Efficiency(const string motherNucl = "37Na", 
  ...
  DecayCascade *dc = new DecayCascade(30, motherNucl);
  ...
  printf("--- Total number of mother nucleus %s: \n\t%.1lf (<10.0x halflife (Total counts in the decay curve 0->5 ms)); \n\t%.1lf (<%.1lfx halflife (Integral of the parent component 0->5 ms))\n",
	 ...
	 hDecayTime->Integral(hDecayTime->FindBin(0), hDecayTime->FindBin(3)),
	 (fMother->Integral(0, 3))//-hDecayTime->GetEntries() )/hDecayTime->GetEntries()
	 ,
	 NHALFLIFE 
	 ); 
  ...
  // 						  f->GetHalflife(),
  // 						  f->GetHalflifeErr()));
  hDecayTime->SetTitle(";Time (ms);Counts/1.5ms"); 
