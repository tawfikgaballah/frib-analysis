...
void TPulser::Unpack(DDASRootFitHit* ddashit, TRandom3 &rng, TPulserCalibrator &cal)
{
  ...
  timecfd = ddashit->getTimeCFD();
  cfdfailbit = ddashit->getCFDFailBit(); // no get();
...
//   - none
//
void TPulser::UnpackFit(DDASRootFitHit* ddashit, TPulserCalibrator &cal)
{
  if (ddashit->hasExtension()) {
    auto ext = ddashit->getExtension();
    ...
    E1real_single = analyticfit::pulseAmplitude(E1_single,E1steepness_single,E1decay_single,T1_single);
    E1real_double = analyticfit::pulseAmplitude(E1_double,E1steepness_double,E1decay_double,T1_double);
    E2real_double = analyticfit::pulseAmplitude(E2_double,E2steepness_double,E2decay_double,T2_double);
