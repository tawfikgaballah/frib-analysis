...
inline bool exists_test(const std::string& name) {
  struct stat buffer;
  int result = stat(name.c_str(), &buffer);
  if (result != 0) {
    std::cerr << "File not found or inaccessible: [" << name << "]\n";
    return false;
  }
  return true;
}
...
int main(int argc, char **argv) 
{
  ...
  // A very quick hack for four digit runnumber strings
  ...
  for(int sr=0; sr<subrun; sr++) {
    ...
    if(sr/*subrun*/ < 10) {
      inputfileName << fileInDir << "run-" << runnumber << "-0" << /*subrun*/sr << "-fitted-converted.root";
	...
	break;
      ...
      inputfileName << fileInDir << "run-" << runnumber << "-" << sr << "-fitted-converted.root";
      if(exists_test(inputfileName.str())) {
  	chain_in->AddFile(inputfileName.str().c_str());
  	std::cout << "Added " << inputfileName.str() << " to TChain" << std::endl;
        havedata = true;
  ...
  outputfileName << fileOutDir << "run-" << runnumber << "-" << subrun << "-sorted.root";
  ...
  if(havedata) {
    ...
    analyzer.AnalyzeData<TChain>(chain_in, tree_out, runnumber);
  }
